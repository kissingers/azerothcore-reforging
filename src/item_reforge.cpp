...
void ItemReforge::HandleReload() const
{
    ...
    for (WorldSessionMgr::SessionMap::const_iterator itr = sessionMap.begin(); itr != sessionMap.end(); ++itr)
	{
        ...
        for (/*iter*/; iter != playerItems.end(); ++iter)
        {
            ...
            if (!item->IsEquipped())
                continue;
            player->_ApplyItemMods(item, item->GetSlot(), false);	//改为每一件穿的装备属性不生效
            player->_ApplyItemMods(item, item->GetSlot(), true);	//然后再属性生效,这样会导致每个属性都会掉一件装备属性最高的点,体现为血量和蓝量上限受影响
        }
	}
}
